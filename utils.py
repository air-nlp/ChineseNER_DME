...
def create_model(session, Model_class, path, load_vec, load_gaz2vec, config, id_to_char, id_to_word, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        if config["pre_emb"]:
            ...
            gaz_weights = session.run(model.word_lookup.read_value())
            gaz_weights = load_gaz2vec(config["gaz_file"], id_to_word, config["char_dim"], gaz_weights)
            session.run(model.word_lookup.assign(gaz_weights))
            logger.info("Load pre-trained  gaz embedding.")
